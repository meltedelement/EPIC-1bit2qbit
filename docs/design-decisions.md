...
**Crypto subprocess split:** All cryptographic operations (Double Ratchet, PQXDH, AES-256-GCM, key management) run in a separate **Python subprocess** (entry point `client/subprocess_handler.py`, dispatching to `client/crypto_functions/`). The C++ binary spawns this subprocess at startup and communicates with it over the subprocess's **stdin/stdout** using newline-delimited JSON — one request line in, one response line out: `{"method": "...", "params": { ... }}` → `{"result": { ... }}` or `{"error": "..."}`. This avoids embedding the CPython interpreter in C++ (which requires managing the GIL and bridging async Python from sync C++), and lets us keep the existing Python crypto implementation without porting it.

> **Transport note (revised):** Earlier design notes specified a **Unix domain socket**. The implementation switched to **stdin/stdout pipes** — equivalent semantics for a synchronous request/response pattern, portable across Windows and Linux, and simpler for the C++ spawn-and-communicate path (no socket path management or `AF_UNIX` setup). The C++ side is `CryptoProxy` (`fork`/`exec` + pipe framing); the Python side is `subprocess_handler.py`. Revisit only if bidirectional streaming becomes necessary.

**TLS:** Raw OpenSSL (`libssl`/`libcrypto`) — not Boost.Beast, which abstracts the TLS handshake away. Direct use of `SSL_CTX_set_verify()`, `SSL_get_peer_certificate()`, and custom verification callbacks allows full control over certificate chain validation and certificate pinning. The server cert must chain to a system-trusted CA (`X509_V_OK` is required before the pin is checked); self-signed certs are rejected. The pin is layered on top of CA verification — this is stronger than pure TOFU, not a substitute for it. The Networks spec explicitly calls out low-level `libssl` usage as the target approach.
